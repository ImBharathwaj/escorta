...
/** POST: extend active sexter session by 5 minutes. Client only; costs 1 credit. */
export async function POST(
  ...
  if (payload.role !== "client") {
    return NextResponse.json(
      { error: "Only clients can extend a session." },
      { status: 403 }
    );
  }
  ...
  return NextResponse.json({
    ...
    message: `Session extended by ${SEXTER_SESSION_MINUTES} minutes.`,
  });
}
